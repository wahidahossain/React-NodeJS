...
exports.list = function (req, res) {
    Course.find().sort('-created').populate('creator', 'firstName lastName fullName username').exec((err, courses) => {
        if (err) {
            return res.status(400).send({
                message: getErrorMessage(err)
            });
...
exports.courseByID = function (req, res, next, id) {
    Course.findById(id).populate('creator', 'firstName lastName fullName username').exec((err, course) => {
...
exports.CoursesByStudent = function (req, res, next) {
    Course.find({ creator: req.params.id}, function (err, courses) {
        if (err) { return getErrorMessage(err); }
        res.status(200).json(courses)
    });
};
